...
% \name{方缙} & \multirow{6}{\includegraphics[scale=0.13]{avatar.jpg}} \\
...
\begin{itemize}[parsep=0.5ex]
  \item 现有针对大模型的分布式训练任务往往考虑如何高效地进行通信调度或资源复用，没有考虑算网资源对任务执行的影响
  \item 调研现有大模型任务部署和算力调度优化方案，熟悉常见模型并行和数据并行工作
  \item 建模不同算力间的通信拓扑、通信链路对任务训练时间的影响，辅助决策大模型训练任务部署
\end{itemize}
...
\begin{itemize}[parsep=0.5ex]
  \item 目前基于端主机的故障注入难以覆盖大量复杂网络故障场景，且无法针对应用流量精准注入故障
...
\begin{itemize}[parsep=0.5ex]
  ...
  \item 基于 Pytorch 实现包含 8 台服务器的 PS 架构分布式模型训练原型系统，主机间通过自定义协议进行通信
